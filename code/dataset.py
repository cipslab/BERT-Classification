...
class PDataset(Dataset):
    ...
    def tokenize(self,text):
        tokens = encoded_text = self.tokenizer.encode(
    ...
    def __getitem__(self, idx):
        if self.mode == "train":
            ...
            label = None if not len(self.test[self.test["story_id"]==story_id]["label"].values) else self.test[self.test["story_id"]==story_id]["label"].values[0]
            token_list = []
            attn_mask = []
